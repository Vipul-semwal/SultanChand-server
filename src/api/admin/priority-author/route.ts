import {
  ...
  import {  PirorityAuthor_MODULE} from "src/modules/priorityAuthor";
  import PriorityAuthorService from 'src/modules/priorityAuthor/service';
 ...
 const authorList = Array.isArray(data?.[0]?.authors?.list)
  ...
  : []
        ...
        const result = await priorityAuthorService.updatePriorityAuthors({
            id: data[0]?.id || "",
            authors: {list:[...authorList, {author_id: req.validatedBody.author_id, priority: data.length + 1}]},
            key: "global-key"
        });
        console.log("result", result);
        return res.json({success:true,message:"added to priority list",result});
